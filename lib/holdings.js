...
async function getHoldings(wallet) {
  ...
  let maxCapacity = 0;
  let actualNftHoldings = 0;
  let tokensAttachedKnown = false;
  ...
  for (const item of assets) {
    ...
    const tokensAttachedAttr = attrs.find(a => (a.trait_type || '').toLowerCase() === 'tokens attached');
    if (tokensAttachedAttr) {
      tokensAttachedKnown = true;
      actualNftHoldings += Number(tokensAttachedAttr.value) || 0;
    }
  }

  return { counts, legendaryNames, maxCapacity, actualNftHoldings: tokensAttachedKnown ? actualNftHoldings : null };
}
