...
// Helius/Magic Eden cache each NFT's off-chain metadata JSON and can serve a stale
// snapshot (confirmed 2026-07-09: ME/Helius showed a bag's flat mint-time 22,500
// 'Tokens Attached' value while DUMPSTR's own metadata endpoint already reflected
// the real, live 500,000 staked). So this value is fetched fresh per-asset here
// rather than trusting content.metadata.attributes.
async function fetchLiveTokensAttached(jsonUri) {
  if (!jsonUri) return null;
  try {
    const res = await fetch(jsonUri);
    if (!res.ok) return null;
    const meta = await res.json();
    const attr = (meta.attributes || []).find(a => (a.trait_type || '').toLowerCase() === 'tokens attached');
    return attr ? Number(attr.value) || 0 : null;
  } catch {
    return null;
  }
}
...
async function getHoldings(wallet) {
  ...
  for (const item of assets) {
    ...
    dumpstrAssets.push(item);
  }

  const liveAmounts = await Promise.all(
    dumpstrAssets.map(item => fetchLiveTokensAttached(item.content?.json_uri))
  );
  const known = liveAmounts.filter(v => v != null);
  const actualNftHoldings = known.length ? known.reduce((sum, v) => sum + v, 0) : null;
